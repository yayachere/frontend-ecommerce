      ...
      <div className="flex flex-col gap-4 w-full sm:max-w-[480px]">
        ...
        <div className="mt-8">
          ...
          <div className="mt-12">
            ...
            <div onClick={()=>setMethod('stripe')} className="flex ga-3 flex-col lg:flex-row">
              <div className="flex items-center gap-3 border p-2 px-3 cursor-pointer">
                <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'stripe' ? 'bg-green-400' : ''}`}></p>
                <img className="h-5 mx-4" src={assets.stripe_logo} alt="" />
              </div>
            </div>
            <div onClick={()=>setMethod('razorpay')} className="flex ga-3 flex-col lg:flex-row">
              <div className="flex items-center gap-3 border p-2 px-3 cursor-pointer">
                <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'razorpay' ? 'bg-green-400' : ''}`}></p>
                <img className="h-5 mx-4" src={assets.razorpay_logo} alt="" />
              </div>
            </div>
            <div onClick={()=>setMethod('cod')} className="flex ga-3 flex-col lg:flex-row">
              <div className="flex items-center gap-3 border p-2 px-3 cursor-pointer">
